...
import es.princip.getp.api.controller.project.query.description.PublicProjectDetailResponseDescription;
...
class ProjectQueryControllerTest extends ControllerTest {
    ...
    class GetProjectByProjectId {
        ...
        @DisplayName("로그인 사용자는 프로젝트의 상세 정보를 조회할 수 있다.")
        void getProject_WhenUserLogined() throws Exception {
            final MemberType memberType = MemberType.ROLE_PEOPLE;
            final ProjectDetailResponse response = ProjectQueryResponseFixture.projectDetailResponse(projectId);
            
            given(getProjectQuery.getDetailBy(memberId, projectId, memberType)).willReturn(response);
        ...
        @Test
        @DisplayName("비로그인 사용자는 프로젝트의 상세 정보를 조회할 수 있다.")
        void getProject_WhenUserNotLogined() throws Exception {
            final PublicProjectDetailResponse response = ProjectQueryResponseFixture.publicProjectDetailResponse(projectId);

            given(getProjectQuery.getPublicDetailBy(projectId)).willReturn(response);

            perform()
                .andExpect(status().isOk())
                .andDo(restDocs.document(
                    pathParameters(parameterWithName("projectId").description("프로젝트 ID")),
                    responseFields(PublicProjectDetailResponseDescription.description())
                    ))
                    .andDo(print());
        }
    }
